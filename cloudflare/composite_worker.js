...
export default{async fetch(req,e,ctx){const u=new URL(req.url);if(req.method==="GET"&&u.pathname==="/telegram/health")return Response.json({ok:true,service:BRAND,mode:"reference-clone-completion-v16",legacy_routes:"preserved",tradingview_core:"delegated",menus:["main","trades","advanced","portfolio","account"]});if(req.method==="GET"&&u.pathname==="/telegram/status")return status(e);if((req.method==="GET"||req.method==="POST")&&u.pathname==="/telegram/register")return register(req,e);if(req.method==="POST"&&u.pathname==="/telegram/webhook")return webhook(req,e,ctx);return legacyWorker.fetch(req,legacyEnv(e),ctx)}};
