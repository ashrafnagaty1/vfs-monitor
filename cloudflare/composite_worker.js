...
const ADV_TOOLS={
 "🔮 التوقع الزمني":kb([["🔮 تحليل زمني محسن","📅 اليوم / غدًا"],["🏆 أفضل 10","🤝 مناطق التوافق"],["📊 دقة التوقع","📖 دليل التوقع الزمني"],["♟️ التحليل المتقدم","🏠 القائمة الرئيسية"]]),
 "🌀 التوقع الفركتالي":kb([["🌀 فحص الفركتال","📈 شارت الفركتال"],["🔎 مسح الأسهم","📖 دليل الفركتال"],["♟️ التحليل المتقدم","🏠 القائمة الرئيسية"]]),
 "🦋 أنماط الهارمونيك":kb([["🦋 فحص Harmonic","📈 شارت Harmonic"],["🔎 مسح الهارمونيك","📖 دليل الهارمونيك"],["♟️ التحليل المتقدم","🏠 القائمة الرئيسية"]]),
 "📐 تحليل جان":kb([["📐 تحليل Gann","📈 شارت Gann"],["🔎 ماسح جان","📖 دليل جان"],["♟️ التحليل المتقدم","🏠 القائمة الرئيسية"]]),
 "🌊 موجات إليوت":kb([["🌊 تحليل Elliott","📈 شارت Elliott"],["🔎 ماسح إليوت","📖 دليل إليوت"],["♟️ التحليل المتقدم","🏠 القائمة الرئيسية"]]),
 "💰 Smart Money":kb([["💰 تحليل SMC","📈 شارت SMC"],["🔎 ماسح SMC","📖 دليل Smart Money"],["♟️ التحليل المتقدم","🏠 القائمة الرئيسية"]])
};
...
const findTech=(s,sym)=>{const x=snap(s),a=arr(x.details).length?arr(x.details):techs(s);return a.find(v=>String(v.symbol||"").toUpperCase()===sym)};
...
function stockActions(e,sym){const root=(e.REFO_WEBAPP_URL||DEFAULT_WEBAPP).replace(/\/$/,"");return [[{text:"📊 شارت السهم",web_app:{url:`${root}/?symbol=${encodeURIComponent(sym)}`}},{text:"♟️ تحليل متقدم",callback_data:`adv:${sym}`}],[{text:"🎯 متابعة الخطة",callback_data:`plan:${sym}`},{text:"🚨 تنبيه سعر",callback_data:`alert:${sym}`}]]}
...
function advancedGuide(t){if(t.includes("الهارمونيك"))return "📖 <b>دليل الهارمونيك</b>\nGartley · Bat · Butterfly · Crab. لا يُعلن النموذج مكتملًا إلا إذا أعاد المحرك نتيجة مؤكدة من OHLCV.";if(t.includes("Smart Money"))return "📖 <b>دليل Smart Money</b>\nOB / FVG / BOS / CHoCH / Premium-Discount هنا خوارزميات OHLCV heuristic وليست Order Book أو تدفق مؤسسي مؤكد.";if(t.includes("جان"))return "📖 <b>دليل جان</b>\nزوايا ودورات هندسية محسوبة من OHLCV عند توفر نتيجة المحرك؛ لا تُختلق مستويات.";if(t.includes("إليوت"))return "📖 <b>دليل إليوت</b>\n1-2-3-4-5 و A-B-C مع نسب Fibonacci فقط عندما يكتشف المحرك بنية مؤكدة.";if(t.includes("الفركتال"))return "📖 <b>دليل الفركتال</b>\nمطابقة أنماط تاريخية واحتمالات مشروطة؛ ليست ضمانًا لمسار السعر.";return "📖 <b>دليل التوقع الزمني</b>\nتوقع احتمالي يعتمد على النتائج التي ينتجها المحرك فقط، ويجب قراءة الثقة والتوافق مع المخاطر."}
...
async function register(req,e){if(!token(e))return new Response("Missing Telegram bot token",{status:503});const o=new URL(req.url).origin,p={url:`${o}/telegram/webhook`,allowed_updates:["message","callback_query"],drop_pending_updates:false};if(e.TELEGRAM_WEBHOOK_SECRET)p.secret_token=e.TELEGRAM_WEBHOOK_SECRET;const r=await fetch(api(e,"setWebhook"),{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(p)}),hook=await r.json().catch(()=>({ok:false}));let commands=null;if(hook.ok){commands=await fetch(api(e,"setMyCommands"),{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({commands:[{command:"start",description:"القائمة الرئيسية"},{command:"menu",description:"عرض القائمة"},{command:"stock",description:"تحليل سهم"},{command:"signal",description:"فرص وإشارات"},{command:"chart",description:"فتح الشارت"},{command:"financial",description:"التقارير المالية"},{command:"alert",description:"التنبيهات"},{command:"watchlist",description:"قائمة المتابعة"},{command:"portfolio",description:"المحفظة"},{command:"help",description:"المساعدة"}]})}).then(x=>x.json()).catch(()=>({ok:false}));}return Response.json({ok:!!hook.ok,webhook:hook,commands});}
...
async function webhook(req,e,ctx){if(!token(e))return new Response("Missing token",{status:503});if(e.TELEGRAM_WEBHOOK_SECRET&&req.headers.get("X-Telegram-Bot-Api-Secret-Token")!==e.TELEGRAM_WEBHOOK_SECRET)return new Response("Forbidden",{status:403});const u=await req.json().catch(()=>({})),m=u.message||{},q=u.callback_query||{},c=String(m?.chat?.id||q?.message?.chat?.id||""),t=String(m?.text||"").trim(),cb=String(q?.data||"");if(!c)return Response.json({ok:true});if(gate(e)&&c!==String(gate(e)))return Response.json({ok:true});if(cb){const s=await state(e),[kind,symRaw]=cb.split(":"),sym=String(symRaw||"").toUpperCase();if(q.id)await tg(e,"answerCallbackQuery",{callback_query_id:q.id}).catch(()=>null);if(kind==="plan"){await sendInline(e,c,stockCard(s,sym),stockActions(e,sym));return Response.json({ok:true,feature:"plan"})}if(kind==="adv"){await send(e,c,`♟️ <b>تحليل متقدم — ${esc(sym)}</b>\nاختر المحرك ثم اختر السهم من قائمة التحليل إذا احتاج المحرك Snapshot مخصصًا.`,ADV);return Response.json({ok:true,feature:"advanced"})}if(kind==="alert"){await send(e,c,`🚨 <b>تنبيه سعر — ${esc(sym)}</b>\nإضافة تنبيه مخصص تحتاج تخزين مستخدم/مستوى سعر موثوق. الميزة لن تنشئ تنبيهًا وهميًا؛ التنبيهات المسجلة حاليًا متاحة من «تنبيهاتي».`,PORT);return Response.json({ok:true,feature:"alert"})}}if(!t)return Response.json({ok:true});if(["/start","/menu","🏠 القائمة الرئيسية","⬅️ العودة للقائمة الرئيسية"].includes(t)){await showMenu(e,c);return Response.json({ok:true,feature:"main"})}if(["🖥️ الشاشة اللحظية","📊 الشاشة اللحظية"].includes(t)){await showLive(e,c);return Response.json({ok:true,feature:"live"})}const s=await state(e);
 ...
 if(Object.prototype.hasOwnProperty.call(ADV_TOOLS,t)){await send(e,c,advancedResult(s,t),ADV_TOOLS[t]);return Response.json({ok:true})}
 if(t.startsWith("📖 دليل")){await send(e,c,advancedGuide(t),ADV);return Response.json({ok:true})}
 if(["🔮 تحليل زمني محسن","📅 اليوم / غدًا","🏆 أفضل 10","🤝 مناطق التوافق","📊 دقة التوقع","🌀 فحص الفركتال","📈 شارت الفركتال","🔎 مسح الأسهم","🦋 فحص Harmonic","📈 شارت Harmonic","🔎 مسح الهارمونيك","📐 تحليل Gann","📈 شارت Gann","🔎 ماسح جان","🌊 تحليل Elliott","📈 شارت Elliott","🔎 ماسح إليوت","💰 تحليل SMC","📈 شارت SMC","🔎 ماسح SMC"].includes(t)){await send(e,c,"🔎 <b>"+esc(t)+"</b>\nاختر السهم. إذا كانت نتيجة المحرك موجودة في Snapshot ستُعرض؛ وإلا ستظهر «غير متاح» بدل نتيجة مصطنعة.",ADV_BACK);return Response.json({ok:true})}
 ...
 if(/^📊\s+[A-Z0-9]+$/.test(t)||/^تحليل\s+[A-Za-z0-9.]+$/i.test(t)){const sym=t.replace(/^📊\s+/,"").replace(/^تحليل\s+/i,"").toUpperCase();await sendInline(e,c,stockCard(s,sym),stockActions(e,sym));await send(e,c,"اختر سهمًا آخر أو ارجع للقائمة:",STOCK_PICK);return Response.json({ok:true})}
...
export default{async fetch(req,e,ctx){const u=new URL(req.url);if(req.method==="GET"&&u.pathname==="/telegram/health")return Response.json({ok:true,service:BRAND,mode:"reference-clone-completion-v21",legacy_routes:"preserved",tradingview_core:"delegated",menus:["main","trades","advanced","portfolio","account"],configured:{telegram:!!token(e),state:!!e.STATE,webapp:!!(e.REFO_WEBAPP_URL||DEFAULT_WEBAPP)}});if(req.method==="GET"&&u.pathname==="/ops/health"){let stateReadable=false,stateError=null;if(e.STATE){try{await e.STATE.get("__refo_health_probe__");stateReadable=true}catch(err){stateError=String(err?.message||err)}}const ok=!!token(e)&&!!e.STATE&&stateReadable;return Response.json({ok,service:BRAND,version:"ops-health-v21",runtime:"cloudflare-worker",telegram:{configured:!!token(e)},state:{bound:!!e.STATE,readable:stateReadable,error:stateError},webapp:{url:e.REFO_WEBAPP_URL||DEFAULT_WEBAPP,configured:!!(e.REFO_WEBAPP_URL||DEFAULT_WEBAPP)},data:{policy:"no-fabrication",official_indices:false,bid_ask:false,depth:false,fundamentals:false},time:new Date().toISOString()},{status:ok?200:503})}if(req.method==="GET"&&u.pathname==="/telegram/status")return status(e);if((req.method==="GET"||req.method==="POST")&&u.pathname==="/telegram/register")return register(req,e);if(req.method==="POST"&&u.pathname==="/telegram/webhook")return webhook(req,e,ctx);return legacyWorker.fetch(req,legacyEnv(e),ctx)}};
